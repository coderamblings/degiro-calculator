...
def process_purchase(row):
    ...
    normalized_name1 = original_name.replace('"','')
    normalized_name2 = normalized_name1.replace("'","")
    #Table name which contains spaces needs to be surrounded by ticks, thus u0060
    table_name = u"\u0060" + normalized_name2 + u"\u0060"
    ...
    stock_name = u"\u0022" + normalized_name2 + u"\u0022"
...
def process_sale(row):
    ...
    for row_s in result_sales:
        ...
        print 'Starting sale "stock_no" of ' + normalized_name2 + ": " + str(row_s["available_stock_no"])
        #End of the operation - all happens within a single row
        if row_s["available_stock_no"] >= stock_to_sale_no:
            print "Now finishing sale of %s, using row ID: %s" % (normalized_name2, row_s["ID"])
            ...
            update_sales_table(row, normalized_name2, sale_value,purchase_cost)
            stock_to_sale_no = 0
            break #if stock_to_sale_no is 0, we're done with this operation

        #Complex path - if the sale is greater than a number of stock in the first visible purchase of that stock, we must sell all that stock, then move onto another row and keep selling
